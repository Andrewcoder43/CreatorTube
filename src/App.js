...
const App = () => {
  const apiKey = "AIzaSyAiYrrgQImSbs7Bf7JmnhbULjAaoT1wYKE"; // Access the API key from the environment variable
  const channelIds = [
      ...
      <div>
        <nav style={{ marginBottom: "20px" }}>
          <ul style={{ listStyleType: "none", padding: 0, display: "flex", gap: "10px" }}>
            <li>
              <Link to="/" style={{ textDecoration: "none", color: "blue" }}>CreatorTube</Link>
            </li>
            <li>
              <Link to="/random-video" style={{ textDecoration: "none", color: "blue" }}>Random Video Generator</Link>
            </li>
          </ul>
        </nav>
        <Routes>
